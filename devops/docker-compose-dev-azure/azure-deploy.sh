...
resourcename="datemaster"  # lower case, only
resourcegroup="datemaster_rg"
...
# git clone https://github.com/apilogicserver/datemaster.git
# cd datemaster
...
az group create --name datemaster_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group datemaster_rg --sku S1 --is-linux
az container create --resource-group datemaster_rg --name datemaster --image apilogicserver/datemaster:latest --dns-name-label datemaster --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//datemaster.westus.azurecontainer.io:5656'
